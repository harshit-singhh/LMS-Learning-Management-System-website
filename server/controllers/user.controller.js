...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  // if we want to give admin role we will give explicitly from DATABASE.
  ...
  // Generating a JWT token in user Model there no need to import
...
export const getLoggedInUserDetails = asyncHandler(async (req, res, _next) => {
  ...
  console.log("this got called after subscription got cancelled.. to fetch user data", user);
  res.status(200).json({
    success: true,
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  // Saving the forgotPassword token to DB
  await user.save();
...
export const changePassword = asyncHandler(async (req, res, next) => {
  ...
  // Setting the password undefined so that it won't get sent in the response ( even if gets sent by mistake, then password is not sent);
  user.password = undefined;
...
//   const { id } = req.params; // we could have extracted the id by this method also
